...
const {
  slStopId,
  slLinesToShow,
  minWalktimeToStopMins,
  windowForGoSignalMins,
} = require('./config.js');
...
async function getSlDepartureInformation() {
  ...
  if (!departures || !Array.isArray(departures))
    throw Error('SL departures error');
...
function parseSlDepartureInformation(departures) {
  const filteredDep = departures
    ...
    .filter(
      (departure) => minsBetween(departure.expected) > minWalktimeToStopMins
    )
    // map all the attributes we need and add a go/not-yet property
    .map((departure) => {
      return {
        goNow:
          minsBetween(departure.expected) <=
          minWalktimeToStopMins + windowForGoSignalMins,
  ...
  if (filteredDep.some((dep) => dep.mode === 'METRO')) {
    metro = filteredDep.some(
      (dep) => dep.goNow === true && dep.mode === 'METRO'
    )
      ? STATUS_GO
      : STATUS_WAIT;
  }
  ...
  if (filteredDep.some((dep) => dep.mode === 'TRAIN')) {
    train = filteredDep.some(
      (dep) => dep.goNow === true && dep.mode === 'TRAIN'
    )
      ? STATUS_GO
      : STATUS_WAIT;
  }
